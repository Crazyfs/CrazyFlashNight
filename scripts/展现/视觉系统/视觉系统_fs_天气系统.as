...
_root.天气系统.获得当前光照等级 = function()
{
    ...
    if((时间 < 4 and 时间 > 1) or (时间 < 13 and 时间 > 11) or (时间 < 21 and 时间 > 18))
    {
        ...
        光照等级 = _root.常用工具函数.线性插值(时间, baseLevel, nextLevel, this.昼夜光照[baseLevel], this.昼夜光照[nextLevel]);
    }
...
_root.天气系统.设置当前天气 = function()
{
    ...
    {
        this.金币时间倍率 = _root.常用工具函数.线性插值(光照等级, 0, this.时间倍率启动等级, this.金币时间最大倍率, 1);
        this.经验时间倍率 = _root.常用工具函数.线性插值(光照等级, 0, this.时间倍率启动等级, this.经验时间最大倍率, 1);
        this.人物信息透明度 = _root.常用工具函数.线性插值(光照等级, 0, this.时间倍率启动等级, 0, 100);
    }
    else
    {
        this.金币时间倍率 = 1;
