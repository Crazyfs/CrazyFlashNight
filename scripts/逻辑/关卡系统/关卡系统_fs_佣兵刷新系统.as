...
_root.生成游戏世界佣兵 = function(添加佣兵函数, 机率, 是否门口)
{
	...
	switch (_root._currentframe)
	{
		...
		case 349 :
		case 405 :
			面积系数 *= 1.2;//基地车库405  基地1层349 地下1层222
			break;
		case 391 :
			面积系数 *= 0.6;//基地顶层391
		case 422 :
	...
	for (var 迭代器 = 0; 迭代器 < 场上佣兵总人数; 迭代器++)
	{
		...
		if (是否门口)
		{
			...
			添加佣兵函数(随机编号,_root.gameworld[刷佣兵的门]._x,_root.gameworld[刷佣兵的门]._y);
		}
		else
		{
			添加佣兵函数(随机编号);
...
_root.场景随机有效位置 = function()
{
	...
	obj = {x:tempx, y:tempy};
	_root.gameworld.localToGlobal(obj);
	if (_root.gameworld.地图.hitTest(obj.x, obj.y, true))
	{
		_root.场景随机有效位置();
	}
	else
...
_root.载入新佣兵库数据 = function(人数, 等级下限, 等级上限, 回调函数, 回调参数)
{
	...
	for (var _loc7_ in list)
	{
		...
		var _loc9_ = [];
		_loc9_.push(_loc8_.level);
		_loc9_.push(_loc8_.name);
		_loc9_.push(_loc8_.id);
		_loc9_.push(_loc8_.height);
		_loc9_.push(_loc8_.face == -1 ? "" : _root.脸型库[_loc8_.face]);
		_loc9_.push(_loc8_.hair == -1 ? "" : _root.发型库[_loc8_.hair]);
		_loc9_.push(_loc8_.equipment.head == -1 ? "" : _root.id物品名对应表[_loc8_.equipment.head]);
		_loc9_.push(_loc8_.equipment.body == -1 ? "" : _root.id物品名对应表[_loc8_.equipment.body]);
		_loc9_.push(_loc8_.equipment.hand == -1 ? "" : _root.id物品名对应表[_loc8_.equipment.hand]);
		_loc9_.push(_loc8_.equipment.leg == -1 ? "" : _root.id物品名对应表[_loc8_.equipment.leg]);
		_loc9_.push(_loc8_.equipment.foot == -1 ? "" : _root.id物品名对应表[_loc8_.equipment.foot]);
		_loc9_.push(_loc8_.equipment.neck == -1 ? "" : _root.id物品名对应表[_loc8_.equipment.neck]);
		_loc9_.push(_loc8_.equipment.primary == -1 ? "" : _root.id物品名对应表[_loc8_.equipment.primary]);
		_loc9_.push(_loc8_.equipment.secondary1 == -1 ? "" : _root.id物品名对应表[_loc8_.equipment.secondary1]);
		_loc9_.push(_loc8_.equipment.secondary2 == -1 ? "" : _root.id物品名对应表[_loc8_.equipment.secondary2]);
		_loc9_.push(_loc8_.equipment.melee == -1 ? "" : _root.id物品名对应表[_loc8_.equipment.melee]);
		_loc9_.push(_loc8_.equipment.gerenade == -1 ? "" : _root.id物品名对应表[_loc8_.equipment.gerenade]);
		_loc9_.push(_loc8_.gender == 1 ? "男" : "女");
		_loc9_.push(_loc8_.level * _root.基础身价值);
