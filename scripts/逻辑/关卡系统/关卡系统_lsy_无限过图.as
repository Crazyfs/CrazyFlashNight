...
_root.无限过图生成敌人 = function(兵种, 敌人实例名, 敌人参数, SpawnIndex, 生存模式出生X, 生存模式出生Y){
	...
	if (Number(SpawnIndex) > -1)
	{
		...
		if(isNaN(生存模式出生X) || isNaN(生存模式出生Y)){
			...
			生存模式出生Y = 出生点.y;
			if(出生点.Identifier){
				生存模式出生Y += 2; //生成位置从出生点向下平移2像素避免被出生点碰撞箱卡住
				游戏世界["door"+SpawnIndex].gotoAndPlay("开门");
			}
...
_root.选关界面进入关卡 = function(关卡难度)
{
	// if (当前关卡名 == "突围")
	// {
	// 	_root.真实宠物信息 = _root.宠物信息;
	// 	_root.宠物信息 = [];
	// 	_root.宠物信息.push([31, 50, 100, 500, 1]);
	// 	_root.宠物信息.push([32, 50, 100, 500, 1]);
	// 	_root.宠物信息.push([33, 50, 100, 500, 1]);
	// 	_root.宠物信息.push([34, 50, 100, 500, 1]);
	// 	_root.宠物信息.push([35, 50, 100, 500, 1]);
	// }
	if (无限过图总关卡)
	{
		_root.无限过图模式关卡计数 = -1;
		_root.无限过图总关卡 = 无限过图总关卡;
		_root.无限过图基本配置 = 无限过图基本配置;
		_root.无限过图实例 = 无限过图实例;
		_root.无限过图出生点 = 无限过图出生点;
		_root.副本动画 = 副本动画;
		_root.副本对话 = 副本对话;
		_root.rogue敌人集合表 = rogue敌人集合表;
	}
	_root.当前通关的关卡 = "";
