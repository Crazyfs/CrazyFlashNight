...
_root.重量速度关系 = function(重量:Number, 等级值:Number):Number 
{
	...
	if (重量 < 0) return 1 + _root.超重惩罚;
	if (重量 <= 等级值) return 1 + _root.超重惩罚 - (重量 / 等级值) * _root.超重惩罚;
	if (重量 > 等级值 && 重量 < 等级值 * 2) return 1;
	if (重量 <= 4 * 等级值) return 1 - ((重量 - 2 * 等级值) / (2 * 等级值)) * _root.超重惩罚;
	return 1 - _root.超重惩罚;
};
...
_root.获取人形怪强化等级 = function(目标人物等级, 目标人物名字) 
{
   //根据等级赋予佣兵强化等级
   ...
   if(目标人物名字 === "Andy" ||目标人物名字 === "Blue" || 目标人物名字 === "Boy" || 目标人物名字 === "Pig" || 目标人物名字 === "King" ){
      ...
      强化等级 = Math.floor((目标人物等级 - 20) / 4);
   }else if(目标人物等级 <= 50){
      强化等级 = 4 + Math.floor((目标人物等级 - 36) / 8);
   }else{
      强化等级 = 6 + Math.floor((目标人物等级 - 50) / 15);
   }
...
_root.刷新人物装扮 = function(目标)
{
	...
	if (是主角)
	{
		...
		头部强化等级 = _root.获得强化等级(_root.头部装备);
		上装强化等级 = _root.获得强化等级(_root.上装装备);
		手部强化等级 = _root.获得强化等级(_root.手部装备);
		下装强化等级 = _root.获得强化等级(_root.下装装备);
		脚部强化等级 = _root.获得强化等级(_root.脚部装备);
		颈部强化等级 = _root.获得强化等级(_root.颈部装备);
		长枪强化等级 = _root.获得强化等级(_root.长枪);
		手枪强化等级 = _root.获得强化等级(_root.手枪);
		手枪2强化等级 = _root.获得强化等级(_root.手枪2);
		刀强化等级 = _root.获得强化等级(_root.刀);
	}
	else
	{
		//根据等级赋予佣兵强化等级
		var 强化等级 = _root.获取人形怪强化等级(目标人物.等级, 目标人物.名字);
		头部装备 = 目标人物.头部装备;
		上装装备 = 目标人物.上装装备;
		手部装备 = 目标人物.手部装备;
		下装装备 = 目标人物.下装装备;
	...
	for (var 迭代器 in 装备数据)
	{
		...
		{
			目标人物.空手攻击力 += _root.强化计算(装备0号数据.punch, 装备迭代器[1]);
		}
	...
	目标人物.防御上限 = _root.根据等级计算值(目标人物.防御上限_min, 目标人物.防御上限_max, 目标人物.等级);
	if (目标人物.防御力 > 目标人物.防御上限)
	{
		目标人物.防御力 = 目标人物.防御上限;
	}
...
_root.主角函数.初始化可用技能 = function()
{
	...
	技能表.push({技能名:"兴奋剂", 点数:10, 冷却:2, 消耗:16, 限制:1, 距离min:200, 距离max:400, 类型:"增益", 功能:"速度"});
	技能表.push({技能名:"小跳", 点数:10, 冷却:1, 消耗:5, 限制:1, 距离min:0, 距离max:100, 类型:"躲避", 功能:"高频位移"});
	技能表.push({技能名:"铁布衫", 点数:20, 冷却:5, 消耗:16, 限制:10, 距离min:200, 距离max:400, 类型:"增益", 功能:"防护"});
	技能表.push({技能名:"闪现", 点数:30, 冷却:2, 消耗:10, 限制:25, 距离min:0, 距离max:100, 类型:"躲避", 功能:"长距位移"});
	技能表.push({技能名:"霸体", 点数:25, 冷却:20, 消耗:40, 限制:15, 距离min:0, 距离max:400, 类型:"增益", 功能:"解围霸体"});
	技能表.push({技能名:"觉醒霸体", 点数:150, 冷却:20, 消耗:60, 限制:30, 距离min:0, 距离max:400, 类型:"增益", 功能:"解围霸体"});
	技能表.push({技能名:"寸拳", 点数:15, 冷却:5, 消耗:20, 限制:1, 距离min:30, 距离max:100, 类型:"格斗", 功能:"爆发输出"});
	技能表.push({技能名:"气功波", 点数:20, 冷却:5, 消耗:20, 限制:1, 距离min:0, 距离max:400, 类型:"格斗", 功能:"远程输出"});
	技能表.push({技能名:"踩人", 点数:15, 冷却:5, 消耗:15, 限制:1, 距离min:0, 距离max:50, 类型:"格斗", 功能:"贴身输出"});
	技能表.push({技能名:"日字冲拳", 点数:20, 冷却:5, 消耗:15, 限制:10, 距离min:30, 距离max:60, 类型:"格斗", 功能:"贴身持续输出"});
	技能表.push({技能名:"组合拳", 点数:20, 冷却:5, 消耗:15, 限制:10, 距离min:30, 距离max:60, 类型:"格斗", 功能:"贴身持续蓄力输出"});
	技能表.push({技能名:"虎拳", 点数:30, 冷却:8, 消耗:30, 限制:30, 距离min:30, 距离max:60, 类型:"格斗", 功能:"贴身持续输出"});
	技能表.push({技能名:"震地", 点数:25, 冷却:5, 消耗:30, 限制:10, 距离min:0, 距离max:100, 类型:"格斗", 功能:"爆发解围输出"});
	技能表.push({技能名:"地震", 点数:60, 冷却:10, 消耗:60, 限制:30, 距离min:0, 距离max:100, 类型:"格斗", 功能:"持续解围输出"});
	//技能表.push({技能名:"觉醒地震", 点数:500, 冷却:25, 消耗:150, 限制:50, 距离min:0, 距离max:100, 类型:"格斗", 功能:"持续解围爆发输出"});
	技能表.push({技能名:"瞬步斩", 点数:25, 冷却:5, 消耗:20, 限制:10, 距离min:50, 距离max:250, 类型:"刀技", 功能:"位移持续输出"});
	技能表.push({技能名:"凶斩", 点数:25, 冷却:5, 消耗:20, 限制:15, 距离min:40, 距离max:80, 类型:"刀技", 功能:"蓄力输出"});
	技能表.push({技能名:"拔刀术", 点数:50, 冷却:6, 消耗:40, 限制:25, 距离min:0, 距离max:400, 类型:"刀技", 功能:"位移持续输出"});
	技能表.push({技能名:"龙斩", 点数:100, 冷却:30, 消耗:80, 限制:35, 距离min:0, 距离max:150, 类型:"刀技", 功能:"解围持续爆发输出"});
	//技能表.push({技能名:"火力支援", 点数:40, 冷却:15, 消耗:40, 限制:25, 距离min:200, 距离max:400, 类型:"火器", 功能:"持续输出"});
	//技能表.push({技能名:"拔刀术", 点数:50, 冷却:12, 消耗:120, 限制:35, 距离min:50, 距离max:400, 类型:"刀技", 功能:"位移持续输出"});
...
_root.主角函数.掉落装备 = function()
{
	...
	if (this.是否为敌人 == true)
	{
		...
		else if (this.颈部装备 === "角斗高手项链" || this.颈部装备 === "角斗王者项链")
		{
			jjcDropItem = ["次品蓝晶", "巨兽", "冰魄斩", "合金", "烈焰", "异形", "巴雷特", "方舟武士"];
			j = 0;
			i = 0;
			while (i < jjcDropItem.length)
			{
				if (this.长枪.indexOf(jjcDropItem[i]) > -1 && random(20) <= 5)
				{
					_root.创建可拾取物(this.长枪,1,this._x,this._y,true);
					j++;
					break;
			...
			while (i < jjcDropItem.length)
			{
				if (this.刀.indexOf(jjcDropItem[i]) > -1 && random(20) <= 5)
				{
					_root.创建可拾取物(this.刀,1,this._x,this._y,true);
					j++;
					break;
			...
			{
				i = 0;
				tmp_jjcDropItem_ratio = random(7);
				while (i < jjcDropItem.length)
				{
					if (this.头部装备.indexOf(jjcDropItem[i]) > -1 && tmp_jjcDropItem_ratio == 1)
					{
						_root.创建可拾取物(this.头部装备,1,this._x,this._y,true);
						j++;
						if (j >= 2)
						{
							break;
					...
					if (this.上装装备.indexOf(jjcDropItem[i]) > -1 && tmp_jjcDropItem_ratio == 2)
					{
						_root.创建可拾取物(this.上装装备,1,this._x,this._y,true);
						j++;
						if (j >= 2)
						{
							break;
					...
					if (this.手部装备.indexOf(jjcDropItem[i]) > -1 && tmp_jjcDropItem_ratio == 3)
					{
						_root.创建可拾取物(this.手部装备,1,this._x,this._y,true);
						j++;
						if (j >= 2)
						{
							break;
					...
					if (this.下装装备.indexOf(jjcDropItem[i]) > -1 && tmp_jjcDropItem_ratio == 4)
					{
						_root.创建可拾取物(this.下装装备,1,this._x,this._y,true);
						j++;
						if (j >= 2)
						{
							break;
					...
					if (this.脚部装备.indexOf(jjcDropItem[i]) > -1 && tmp_jjcDropItem_ratio == 5)
					{
						_root.创建可拾取物(this.脚部装备,1,this._x,this._y,true);
						j++;
						if (j >= 2)
						{
							break;
...
_root.主角函数.长枪射击 = function(枪口位置:MovieClip, 子弹属性:Object)
{
	...
	if(this.自瞄中){
		...
		if(敌方目标.hp >0){
			...
			子弹属性.速度X = 速度X;
			子弹属性.速度Y = 速度Y;
			子弹属性.ZY比例 = 敌方目标.Z轴坐标 / (敌方目标._y - 50);
		}else{
			子弹属性.速度X = undefined;
			子弹属性.速度Y = undefined;
			子弹属性.ZY比例 = undefined;
...
{
	if (this.手枪2射击次数[this.手枪2] >= this.手枪2弹匣容量) return false;
	this.手枪2射击次数[this.手枪2]++;
	this.刷新枪口位置(枪口位置,子弹属性);
	if(this.状态.indexOf('行走') > -1){
		子弹属性.子弹散射度 = 子弹属性.移动子弹散射度;
	}else{
...
_root.主角函数.释放技能 = function(技能名, 消耗mp, 技能按键值){
	...
	// function ArrIncludeSkill(arr, skill){
	// 	for(var i = 0; i < arr.length; i++){
	// 		if (arr[i] === skill) return true;
	// 	}
	// 	return false;
	// }
