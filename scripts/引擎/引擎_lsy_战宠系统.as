	...
	for (var i = 0; i < _root.宠物信息.length; i++)
	{
		...
		if (当前宠物信息[4] == 1)
		{
			if (当前宠物信息[2] > 0)
			{
				...
				宠物属性.宠物信息数组号 = i;
				var 宠物对象 = _root.gameworld.attachMovie(宠物兵种,宠物实例名,_root.gameworld.getNextHighestDepth(),{等级:宠物等级, 名字:宠物名字, 宠物属性:宠物属性, 是否为敌人:宠物是否为敌人, 身高:宠物身高, _x:地点X, _y:地点Y});
				宠物mc库.push(宠物对象);
				if (当前宠物信息[0] == 66)
				{
					宠物对象.长枪 = "L85A1";
...
_root.宠物减体力1 = function()
{
	var _loc2_ = 0;
	while (_loc2_ < _root.出战宠物id库.length)
	{
		_root.宠物信息[出战宠物id库[_loc2_]][2] -= 2;
		if (_root.宠物信息[出战宠物id库[_loc2_]][2] < 0)
		{
			_root.宠物信息[出战宠物id库[_loc2_]][2] = 0;
		}
		_loc2_ += 1;
	}
}
_root.宠物减体力 = _root.宠物减体力1;
