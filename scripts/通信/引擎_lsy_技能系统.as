	...
	for(var i = 0; i < 主角技能表.length; i++)
	{
		...
		if (技能[0] == 技能名)
		{
			...
			if (技能[1] < 等级)
			{
				...
				if(_root.技能表对象[技能名].Passive){
					_root.更新主角被动技能();
				}
				return true;
			}
		}
/*		else if (技能[0] == "" and !已获得该技能)
		{
			技能[0] = 技能名;
			技能[1] = 等级;
			技能[2] = "false";
	...
	for(var i = 0; i < 主角技能表.length; i++)
	{
		...
		if (技能[0] == "" and !已获得该技能)
		{
			...
			技能[1] = 等级;
			技能[2] = "false";
			技能[3] = _root.技能表对象[技能名].Type;
			技能[4] = false;
			if(_root.技能表对象[技能名].Equippable){
				技能[4] = true;//Equippable
			}
			_root.排列技能图标();
			_root.发布消息(_root.获得翻译(技能名) + "，" + _root.获得翻译("新技能获得！"));
			if(_root.技能表对象[技能名].Passive){
...
for (var i = 0; i < _root.主角技能表总数; i++)
{
	_root.主角技能表[i] = ["", 0, "false","",true];
	
}

_root.blue的技能 = new Array(60);
...
_root.andy的技能[0] = ["瞬步斩", 1, false];
_root.andy的技能[1] = ["凶斩", 1, false];
_root.andy的技能[2] = ["火舞旋风", 1, false];
_root.andy的技能[3] = ["拔刀术", 1, false];
_root.andy的技能[4] = ["龙斩", 1, false];
_root.andy的技能[5] = ["六连", 1, false];
_root.andy的技能[6] = ["空间斩", 1, false];
